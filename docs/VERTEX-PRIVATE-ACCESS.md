...
| **devai-ai-gateway (nginx)** | `/vertex/*` route to `aiplatform.googleapis.com` — the gateway **injects `x-goog-api-key`** (GCP SM `prod-devai-vertex-api-key` via ExternalSecret, key validated live on both express and project paths) and strips caller auth, same pattern as `/anthropic`. devai-api dials it via `DEVAI_VERTEX_BASE_URL`; pods carry no Vertex credentials. Rides the PSC DNS pin automatically. |
