...
-- deliveries: the table has never had timestamps.
--
-- services/delivery_failure_reconcile.go — the sweep that catches an order
-- stranded by a failed/returned delivery — filters and orders on
-- `deliveries.updated_at`. That column has never existed (the GORM model
-- declares no CreatedAt/UpdatedAt), so all four of its queries have been
-- failing with SQLSTATE 42703 on every 10-minute tick and the safety net has
-- never once run. A delivery that fails with no webhook follow-up therefore
-- strands its order indefinitely, which is exactly the failure mode that left
-- order HC26072808359105 sitting at `ready` with no carrier.
--
-- DEFAULT now() rather than a back-dated value is deliberate: every existing
-- row starts life looking "just touched", so it sits outside the grace window
-- for one grace period instead of the sweep waking up and freezing every
-- historical failed delivery at once.
ALTER TABLE public.deliveries
  ADD COLUMN IF NOT EXISTS created_at timestamp with time zone DEFAULT now() NOT NULL,
  ADD COLUMN IF NOT EXISTS updated_at timestamp with time zone DEFAULT now() NOT NULL;

-- Bump updated_at in the DATABASE rather than relying on GORM's autoUpdateTime.
--
-- Two reasons. First, ordering: adding UpdatedAt to the Go model before this
-- column exists would make GORM emit `updated_at` in every INSERT/UPDATE on
-- deliveries and break every delivery write in prod — so the app-side fix could
-- not ship until this had landed and applied. A trigger fixes the sweep in one
-- merge, in one repo, with no cross-repo deploy ordering to get wrong.
--
-- Second, coverage: the sweep's grace window is only meaningful if the
-- timestamp is bumped by every provider webhook re-fire. A trigger holds that
-- guarantee for any writer, not just the ones going through GORM.
...
-- The sweep scans failed/returned rows oldest-first inside a grace window;
-- without this it degrades to a full scan of every delivery ever made.
CREATE INDEX IF NOT EXISTS ix_deliveries_status_updated
  ON public.deliveries (status, updated_at);
