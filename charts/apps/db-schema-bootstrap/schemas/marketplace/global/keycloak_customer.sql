...
CREATE TABLE IF NOT EXISTS public.org_domain (
    id character varying(36) NOT NULL,
    name character varying(255) NOT NULL,
...
CREATE INDEX IF NOT EXISTS fed_user_attr_long_values ON public.fed_user_attribute USING btree (long_value_hash, name);
...
CREATE INDEX IF NOT EXISTS fed_user_attr_long_values_lower_case ON public.fed_user_attribute USING btree (long_value_hash_lower_case, name);
...
CREATE INDEX IF NOT EXISTS idx_client_att_by_name_value ON public.client_attributes USING btree (name, substr(value, 1, 255));
...
CREATE INDEX IF NOT EXISTS idx_idp_for_login ON public.identity_provider USING btree (realm_id, enabled, link_only, hide_on_login, organization_id);
...
CREATE INDEX IF NOT EXISTS idx_idp_realm_org ON public.identity_provider USING btree (realm_id, organization_id);
...
CREATE INDEX IF NOT EXISTS idx_offline_uss_by_broker_session_id ON public.offline_user_session USING btree (broker_session_id, realm_id);
...
CREATE INDEX IF NOT EXISTS idx_offline_uss_by_last_session_refresh ON public.offline_user_session USING btree (realm_id, offline_flag, last_session_refresh);
...
CREATE INDEX IF NOT EXISTS idx_org_domain_org_id ON public.org_domain USING btree (org_id);
...
CREATE INDEX IF NOT EXISTS idx_perm_ticket_owner ON public.resource_server_perm_ticket USING btree (owner);
...
CREATE INDEX IF NOT EXISTS idx_perm_ticket_requester ON public.resource_server_perm_ticket USING btree (requester);
...
CREATE INDEX IF NOT EXISTS idx_rev_token_on_expire ON public.revoked_token USING btree (expire);
...
CREATE INDEX IF NOT EXISTS idx_usconsent_scope_id ON public.user_consent_client_scope USING btree (scope_id);
...
CREATE INDEX IF NOT EXISTS user_attr_long_values ON public.user_attribute USING btree (long_value_hash, name);
...
CREATE INDEX IF NOT EXISTS user_attr_long_values_lower_case ON public.user_attribute USING btree (long_value_hash_lower_case, name);
